...
export default function VerificationModal({
    ...
    const [category, setCategory] = useState(initialData.category || 'Comida y Bebidas / Restaurante');
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        if (!businessDescription.trim()) {
            setErrorMsg("Por favor indica de qué se trata tu negocio (descripción breve).");
            return;
        }

        if (!category.trim()) {
            setErrorMsg("Por favor selecciona la categoría de tu negocio.");
            return;
        }
        ...
        <div className="fixed inset-0 bg-slate-950/70 backdrop-blur-sm z-50 flex items-center justify-center p-4 overflow-y-auto">
            <div className="bg-white rounded-[36px] max-w-2xl w-full my-8 shadow-2xl border border-slate-100 overflow-hidden flex flex-col max-h-[92vh]">
                ...
                <form onSubmit={handleSubmit} className="p-6 md:p-8 space-y-6 overflow-y-auto flex-1 custom-scrollbar">
                    ...
                    {/* Section 5: Business Activity & Category */}
                    <div className="space-y-4 pt-2 border-t border-slate-100">
                        <div className="flex items-center justify-between">
                            <h3 className="text-sm font-black text-slate-900 uppercase tracking-wider flex items-center gap-2">
                                <Store className="w-4 h-4 text-primary" />
                                5. Actividad y Categoría del Negocio
                            </h3>
                            <span className="text-[11px] font-bold text-amber-600 bg-amber-50 px-2 py-0.5 rounded-md">Obligatorio</span>
                        </div>
                        ...
                        <div className="space-y-1.5">
                            ...
                            <select
                                ...
                                className="w-full bg-slate-50 border border-slate-200 focus:border-primary focus:bg-white p-3.5 rounded-2xl outline-none font-bold text-sm text-slate-800 transition-all cursor-pointer"
                            >
                        ...
                        <div className="space-y-1.5">
                            ...
                            <textarea
                                ...
                                rows={3}
                                placeholder="Describe brevemente los productos, platos o servicios que ofreces a tus clientes..."
                                className="w-full bg-slate-50 border border-slate-200 focus:border-primary focus:bg-white p-3.5 rounded-2xl outline-none font-bold text-sm text-slate-800 transition-all resize-none"
                            />
                            <p className="text-[11px] text-slate-400 font-medium ml-1">
                                Esta descripción nos ayuda a verificar y clasificar correctamente tu comercio para los clientes.
                            </p>
                        </div>
                    </div>
