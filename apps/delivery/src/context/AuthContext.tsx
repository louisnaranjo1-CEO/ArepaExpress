...
export const AuthProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
    ...
    // Unique device session token for this delivery instance
    const getDeviceId = () => {
        let id = localStorage.getItem('deliexpress_driver_device_id');
        if (!id) {
            id = 'drv_' + Math.random().toString(36).substring(2, 12) + '_' + Date.now().toString(36);
            localStorage.setItem('deliexpress_driver_device_id', id);
        }
        return id;
    };
        ...
        <AuthContext.Provider value={{ user, userData, loading, isProfileComplete, isUnlocked, setIsUnlocked, currentLocation }}>
            ...
            {sessionTerminated && (
                <div className="fixed inset-0 z-[9999] flex items-center justify-center p-6 bg-black/80 backdrop-blur-md">
                    <div className="bg-slate-900 border border-slate-800 rounded-[32px] w-full max-w-sm p-8 text-center shadow-2xl animate-in zoom-in-95 duration-200">
                        <div className="w-16 h-16 bg-amber-500/10 text-amber-500 rounded-full flex items-center justify-center mx-auto mb-4">
                            <svg className="w-8 h-8" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z" />
                            </svg>
                        </div>
                        <h3 className="text-xl font-black text-white mb-2">Sesión iniciada en otro equipo</h3>
                        <p className="text-sm text-slate-400 mb-6 font-medium leading-relaxed">
                            Tu cuenta de chofer se acaba de abrir en otro teléfono. Por seguridad operacional, esta sesión ha sido cerrada.
                        </p>
                        <button
                            onClick={() => {
                            ...
                            className="w-full bg-primary text-slate-950 py-4 rounded-2xl font-black shadow-lg shadow-primary/20"
                        >
                            Entendido
                        </button>
                    </div>
