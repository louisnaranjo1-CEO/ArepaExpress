...
export default function CashierDashboard() {
    ...
    const toggleProductAvailability = async (productId: string, currentStatus: boolean) => {
        try {
            ...
            setPosProducts(prev => prev.map(p => p.id === productId ? { ...p, is_available: nextStatus } : p));
            toast.success(nextStatus ? "Producto disponible para clientes" : "Producto marcado como Agotado");
        } catch (e) {
            console.error("Error toggling product availability:", e);
...
`;
                ...
                <div className="flex justify-between items-start mb-4">
                    ...
                    <div className="flex items-center gap-2">
                        <button
                            ...
                            className="p-2 px-3 bg-primary/10 hover:bg-primary/20 text-slate-900 rounded-2xl font-black text-xs flex items-center gap-1.5 transition-all cursor-pointer shadow-xs active:scale-95"
                            title="Abrir Chat en vivo con Cliente"
                        >
                            <MessageSquare className="w-3.5 h-3.5 text-slate-900" />
                            <span className="text-[11px] font-black">Chat</span>
                        </button>
    ...
    return (
        <div className="min-h-screen bg-slate-50 flex flex-col">
            ...
            <header className="bg-white px-6 py-4 flex items-center justify-between border-b border-slate-200 sticky top-0 z-40">
                ...
                <div className="flex items-center gap-3">
                    <button
                        ...
                        className="flex items-center gap-2 px-4 py-2 bg-slate-100 hover:bg-slate-200 text-slate-700 rounded-xl font-bold transition-colors cursor-pointer"
                        title="Control de disponibilidad de productos"
                    >
                        <Utensils className="w-4 h-4 text-slate-600" />
                        <span className="hidden sm:inline">Disponibilidad / Stock</span>
                    </button>
                    <button
            ...
            {/* Modal de Control de Stock y Disponibilidad (Precios Bloqueados) */}
            {showStockModal && (
                <div className="fixed inset-0 z-[150] flex items-center justify-center bg-slate-900/60 backdrop-blur-sm p-4">
                    <div className="bg-white rounded-[32px] p-6 sm:p-8 w-full max-w-2xl shadow-2xl border border-slate-100 max-h-[85vh] flex flex-col animate-in zoom-in-95 duration-200">
                        <div className="flex justify-between items-start pb-4 border-b border-slate-100">
                            <div>
                                ...
                                <p className="text-xs text-slate-400 font-medium mt-1 flex items-center gap-1">
                                    ...
                                    Los precios base están protegidos y solo pueden ser modificados por el Administrador.
                                </p>
                            </div>
                            <button
                        ...
                        <div className="flex-1 overflow-y-auto py-4 space-y-3 custom-scrollbar">
                            {posProducts.length === 0 ? (
                                <p className="text-center text-sm text-slate-400 py-10 font-bold">No hay productos registrados en el menú.</p>
                            ) : (
                                    ...
                                    return (
                                        <div
                                            ...
                                            className={`p-3.5 rounded-2xl border flex items-center justify-between gap-3 transition-all ${
                                                isAvailable ? 'bg-white border-slate-200' : 'bg-rose-50/50 border-rose-100'
                                            }`}
                                        >
                                            <div className="flex items-center gap-3 min-w-0">
                                                ...
                                                <div className="min-w-0">
                                                    <p className={`font-black text-sm truncate ${isAvailable ? 'text-slate-800' : 'text-slate-400 line-through'}`}>
                                                        {product.name}
                                                    </p>
                                            ...
                                            <div className="flex items-center gap-3 shrink-0">
                                                {/* Locked Price Badge */}
                                                <div className="flex items-center gap-1 bg-slate-100 px-3 py-1.5 rounded-xl border border-slate-200" title="Precio base protegido (solo lectura)">
                                                ...
                                                {/* Toggle Availability */}
                                                <button
                                                    onClick={() => toggleProductAvailability(product.id, isAvailable)}
                                                    className={`px-3.5 py-1.5 rounded-xl text-xs font-black transition-all cursor-pointer active:scale-95 ${
                                                        isAvailable
                        ...
                        <div className="pt-4 border-t border-slate-100 flex items-center justify-end">
                            <button
                                onClick={() => setShowStockModal(false)}
                                className="px-6 py-3 bg-slate-900 hover:bg-slate-800 text-white font-black text-xs rounded-xl shadow-lg shadow-slate-900/10 cursor-pointer"
                            >
                                Listo
                            </button>
                        </div>
