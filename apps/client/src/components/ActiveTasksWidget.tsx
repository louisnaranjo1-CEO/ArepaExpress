...
export default function ActiveTasksWidget() {
    ...
    const userId = user?.id || user?.uid || (userData as any)?.id || (userData as any)?.uid || localStorage.getItem('deliexpress_guest_id');
    ...
    const fetchActiveTasks = useCallback(async () => {
        ...
        try {
            ...
            const { data: transports, error: trErr } = await supabase
            ...
            const mandadoReqIds = (transports || [])
                .filter((t: any) => t.service_category === 'muchacho_mandado' && t.status === 'searching')
                .map((t: any) => t.id);
            ...
            (transports || []).forEach((t: any) => {
                ...
                if (t.service_category === 'muchacho_mandado') {
                    if (t.status === 'searching') {
                        if (bids.length > 0) {
                            subtitle = `🔥 ¡${bids.length} ${bids.length === 1 ? 'piloto ha ofertado' : 'pilotos han ofertado'}! Revisa las tarifas abajo y elige:`;
                            badgeStatus = `${bids.length} ${bids.length === 1 ? 'Oferta' : 'Ofertas'}`;
                            badgeColor = 'bg-emerald-400 text-slate-950 font-black animate-pulse';
                        } else {
                            subtitle = 'Escaneando pilotos disponibles en la zona para tu mandado...';
                            badgeStatus = 'Buscando Pilotos';
                ...
                } else if (t.service_category === 'delivery_envios') {
                    if (t.status === 'searching') {
                        subtitle = 'Buscando repartidor disponible para tu paquete...';
                        badgeStatus = 'Buscando Repartidor';
                        badgeColor = 'bg-amber-400 text-slate-950 font-black';
                    } else if (t.status === 'accepted') {
                        subtitle = t.driver_name
                    ...
                    if (t.status === 'searching') {
                        if (t.assigned_driver_id) {
                            ...
                            subtitle = 'Buscando conductor más cercano a tu punto de recogida...';
    ...
    // Handle Synchronized Cancellation
    const handleConfirmCancel = async () => {
        ...
        try {
            ...
            if (taskToCancel.type === 'transport') {
                ...
                // If mandado, delete bids to dismiss driver notifications
                if (taskToCancel.serviceCategory === 'muchacho_mandado') {
                    await supabase
                        .from('transport_bids')
                        .delete()
        ...
        <section className="px-5 mt-3 mb-3 animate-in fade-in slide-in-from-top-2">
            <div className="space-y-3">
                {tasks.map((task) => {
                    ...
                    const isMandado = task.serviceCategory === 'muchacho_mandado';
                    ...
                    return (
                        ...
                        >
                            ...
                            >
                                ...
                                <div className="flex items-center justify-between gap-2 mb-2">
                                    ...
                                    <div className="flex items-center gap-1.5 shrink-0">
                                        <span
                                            className={`text-[10px] font-black uppercase tracking-wider px-2.5 py-1 rounded-full shadow-sm ${task.badgeColor}`}
                                        >
                                            {task.badgeStatus}
                                        </span>
                                ...
                                <p
                                    className={`text-xs font-bold leading-snug mb-2 ${
                                        isArriving || isInProgress ? 'text-slate-100' : 'text-slate-900'
                                    }`}
                                >
                                    {task.subtitle}
                                </p>
                                    ...
                                    <div className="flex items-center justify-between mb-2.5">
                                        <div className="flex items-center gap-1.5">
                                            ...
                                            <span className="text-xs font-black text-amber-400 uppercase tracking-wider">
                                                Ofertas de Pilotos ({task.bids?.length})
                                            </span>
                                        </div>
                            ...
                            {/* Bottom Action Bar: Cancel Button (Synchronized) */}
                            {isCancellable && (
                                <div
                                    className={`px-4 py-2 flex items-center justify-between border-t text-[11px] ${
                                        isArriving || isInProgress
                                ...
                                >
                                    ...
                                    <button
                                        ...
                                        className="inline-flex items-center gap-1 px-2.5 py-1 rounded-lg bg-rose-500/15 hover:bg-rose-500/25 text-rose-700 font-black text-[10px] uppercase tracking-wider border border-rose-400/40 active:scale-95 transition-all"
                                    >
                    ...
                    <div className="bg-white rounded-3xl p-5 max-w-sm w-full shadow-2xl space-y-3.5 border border-slate-200">
                        ...
                        {taskToCancel.status === 'arriving' ? (
                            ...
                            <p className="text-xs text-slate-600 font-medium leading-relaxed">
                                Esta acción cancelará tu solicitud de inmediato en el sistema, liberará a los conductores y retirará la alerta en todas las pantallas.
                            </p>
                        )}
