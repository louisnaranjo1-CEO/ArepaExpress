...
filepath = "src/delivery/pages/OrdersRadar.tsx"
with open(filepath, "r", encoding="utf-8") as f:
    content = f.read()

# 1. Active Order/Transport
old_active = """    // 2. Escuchar órdenes de delivery y transporte activo
    useEffect(() => {
        if (!user) return;
...
new_active = """    // 2. Escuchar órdenes de delivery y transporte activo (Supabase)
    useEffect(() => {
        if (!user) return;
...
old_available = """    // 3. Escuchar viajes disponibles filtrados por vehicleType
    useEffect(() => {
...
new_available = """    // 3. Escuchar viajes disponibles filtrados por vehicleType
    useEffect(() => {
        if (!driverProfile?.vehicleType) {
            setAvailableTransport([]);
            setLoading(false);
...
old_feedback = """    // 5. Escuchar último feedback (calificación)
    useEffect(() => {
        if (!user) return;
...
new_feedback = """    // 5. Escuchar último feedback (calificación)
    useEffect(() => {
        if (!user) return;
